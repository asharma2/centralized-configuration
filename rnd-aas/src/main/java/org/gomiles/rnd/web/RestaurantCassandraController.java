...
@RestController
@RequestMapping("/api/cassandra/restaurant")
public class RestaurantCassandraController {

	private static final Logger LOG = LoggerFactory.getLogger(RestaurantCassandraController.class);

	@Autowired
	RestaurantCassandraRepository restaurantRepository;
	...
	public Restaurant findById(@PathVariable("id") String id) throws GoMilesException {
		...
		return restaurantRepository.findById(UUID.fromString(id)).get();
	}
	...
	public Restaurant delete(@PathVariable("id") String id) throws GoMilesException {
		...
		Restaurant restaurant = restaurantRepository.findById(UUID.fromString(id)).get();
		restaurantRepository.save(restaurant);
		return restaurant;
	}
